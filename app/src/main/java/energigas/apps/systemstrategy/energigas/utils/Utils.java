...
public class Utils {
    ...
    public static void saveEntidad(Context context, DEEntidad deEntidad) {
        SharedPreferences.Editor editor = context.getSharedPreferences(Constants.SESSION_ENTIDAD_ID, Context.MODE_PRIVATE).edit();

        editor.putLong(Constants.ENTIDAD_ID, deEntidad.getEntidadId());

        editor.commit();
    }

    public static Long getEntidad(Context context){

        SharedPreferences prefs = context.getSharedPreferences(Constants.SESSION_ENTIDAD_ID, Context.MODE_PRIVATE);

        return prefs.getLong(Constants.ENTIDAD_ID, 0);
    }
}
