...
public class Summary {
    ...
    public static Summary getListSummary(Context context) {
        ...
        Long mstr_Liquidacion = Session.getCajaLiquidacion(context).getLiqId();

        CajaLiquidacion cajaLiquidacion = CajaLiquidacion.getCajaLiquidacion(mstr_Liquidacion + "");
        ...
        double mint_Saldoinicial = cajaLiquidacion.getSaldoInicial();
        ...
        for (ComprobanteVenta comprobanteVenta : comprobanteVentaList) {
            ...
            if (comprobanteVenta.getTipoComprobanteId() == Constants.TIPO_DOCUMENTO_BOLETA) {
                ...
                ingresos[1] = Double.parseDouble(comprobanteVenta.getFormaPagoId() + "");
                ingresos[2] = Double.parseDouble(comprobanteVenta.getLqId() + "");
                ingresos[3] = Double.parseDouble(comprobanteVenta.getEstadoId() + "");
            ...
            if (comprobanteVenta.getTipoComprobanteId() == Constants.TIPO_DOCUMENTO_FACTURA) {
                ...
                Log.d(TAG, "getLqId: " + comprobanteVenta.getLqId() +"");
            }
        ...
        double mdou_facSumaCobrada = 0.00;

        double bolSumaEmitidos = 0.00;
        double bolSumaPagados = 0.00;
        double mdou_bolSumaCobrada = 0.00;
        ...
        for (Double[] doubles : doubleListBoletas) {
            ...
            Log.d(TAG, "bolSumaEmitidos" + doubles[0]);
        }
        ...
        for (Double[] doubles : doubleListFacturas) {
            ...
            Log.d(TAG, "mstr_Liquidacion" + mstr_Liquidacion);
            Log.d(TAG, "doubles[3].intValue()" + doubles[3].intValue());
        }



        List<SummaryIncome> summaryIncomeList = new ArrayList<>();
        SummaryIncome summaryFacturasEmitidas = new SummaryIncome("Facturas Emitidas", doubleListFacturas.size() + "", Utils.formatDouble(facSumaEmitidos), Utils.formatDouble(facSumaPagados), Utils.formatDouble(mdou_facSumaCobrada));
        SummaryIncome summaryBoletasEmitidas = new SummaryIncome("Boletas Emitidas", doubleListBoletas.size() + "", Utils.formatDouble(bolSumaEmitidos), Utils.formatDouble(bolSumaPagados), Utils.formatDouble(mdou_bolSumaCobrada));
        ...
        ingresosImporte = facSumaPagados + bolSumaPagados;
        double efectivoRendir = mint_Saldoinicial + ingresosImporte - importeGastos;
        //double saldoInicialMain = (cajaLiquidacion.getSaldoInicial() + ingresosImporte) - importeGastos;

        Summary sumaryResumen = new Summary(Utils.formatDoubleNumber(mint_Saldoinicial),
