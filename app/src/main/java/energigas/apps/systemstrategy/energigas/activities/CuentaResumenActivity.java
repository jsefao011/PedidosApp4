...
public class CuentaResumenActivity extends AppCompatActivity implements OnAsyntaskListener, OnLocationListener {
    ...
    private void initDialogCerrarLiquidacion() {
        new DialogGeneral(this).setCancelable(false).setTextButtons("SI", "NO").setMessages("ATENCION...!!!", "¿ Esta seguro de cerrar caja ?").showDialog(new DialogGeneralListener() {
            ...
            public void onSavePressed(AlertDialog alertDialog) {
                ...
                /*
                ///comprovacion de si las tablas s e esportaron complemtmente
                List<SyncEstado> TablCreadas = SyncEstado.find(SyncEstado.class, "NOMBRE_TABLA != 'Be_Doc_Electronico' AND ESTADO_SYNC = ?",new String[]{Constants.S_CREADO+""});

                if(TablCreadas == null)
                {
                    new AsynCerrarCaja(getApplicationContext(), CuentaResumenActivity.this).execute(cajaLiquidacion.getLiqId() + "", location.getLatitude() + "", location.getLongitude() + "");
                }
                else if(TablCreadas.size() == 0){
                    new AsynCerrarCaja(getApplicationContext(), CuentaResumenActivity.this).execute(cajaLiquidacion.getLiqId() + "", location.getLatitude() + "", location.getLongitude() + "");
                }else
                {
                    //Log.d(TAG, "Jse Exportnado Datos");
                    //Intent intentExportService = new Intent(getApplicationContext(), SyncData.class);
                    //intentExportService.setAction(Constants.ACTION_EXPORT_SERVICE);
                    //getApplicationContext().startService(intentExportService);
                    msg_ImportarData();
                    progressDialog.hide();
                }
                */
    ...
    private void msg_ImportarData() {
        Toast.makeText(this, "No se exporto los datos al Servidor", Toast.LENGTH_SHORT).show();
    }
